<script setup name="ProductConfiguration">
...
const handleSubmit = () => {
  proxy.$refs.form.validate((valid) => {
    if (valid) {
      ...
      const { id } = form;
      const api = id !== null ? updateManage : addManage;
      api(form)
        .then(() => {
...
const handleDelete = (row) => {
  const { name } = row;
  proxy.$modal.confirm(`确认要删除产品：${name}吗？`)
    .then(() => {
      list.loading = true;
      delManage(row.id)
        .then(() => {
  ...
  <el-card :class="$s.Card">
    ...
    >
      ...
      <el-table-column align="right">
        <template #header>
          <el-input v-model="keywords" placeholder="产品名称/编号" />
        </template>
